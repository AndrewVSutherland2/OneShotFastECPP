...
(All products of primes are built by balanced pairing -- balanced_product below
-- rather than sequential accumulation, whose cost is quadratic in the product
size; with that, the stated bound holds as implemented, including all product
building in the batched trial division.)
"""
...
def prime_divisors(m, primes, batch_bits):
    ...
    remainder tree splits that down to each prime.  Batch products are built by
    balanced pairing.  One batch is resident at a time, so memory stays
    O(sieve) = O(n^2) bits."""
    out = []
    batch = []
    bits = 0
...
def check_orders(XQ, ZQ, primes, A, p):
    ...
    hL = balanced_product(L)
    hR = balanced_product(Rr)

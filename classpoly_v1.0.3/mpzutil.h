...
static inline long fast_prime_enum_powers (prime_enum_ctx_t *ctx) { return prime_power_enum (ctx); }
...
unsigned long ui_binomial (unsigned n, unsigned k);                   // only handles n <= 28
...
#define ui_lg_floor(x)  ui_highbit(x)                             // floor(lg(x))

...
int torsor_setup (torsor_t T, classpoly_t H, long u)
{
	...
	for ( i = 0 ; i < pres->k ; i++ ) {
		...
		for ( j = 0 ; j < i ; j++ ) e1[j] = ( T->signs[j] < 0 ? pres->o[j] - pres->orient_reps[i][j] : pres->orient_reps[i][j] );
		e1[j] = pres->orient_reps[i][j];  for ( j++ ;  j < pres->k ; j++ ) e1[j] = 0;
		...
		for ( j = 0 ; j < i ; j++ ) e1[j] = ( T->signs[i]*T->signs[j] < 0 ? pres->o[j] -evec_ri(pres->r,i)[j] : evec_ri(pres->r,i)[j] );
		for ( ; j < pres->k ; j++ ) e1[j] = 0;
		evec_reduce (e1, pres->n, T->r, pres->k);
...
int classgroup_pcp_orient (classgroup_pcp_t pres)
{
	...
	for ( i = 0 ; i < pres->k ; i++ ) {
		...
		while ( ! ps[i] ) {
			...
			for ( j = 0 ; j < i ; j++ ) e2[j] = e1[j];
			e2[j] = pres->o[j]-e1[i];  for ( j++ ; j < pres->k ; j++ ) e2[j] = 0;
			evec_reduce(e2,pres->n,pres->r,pres->k);
			for ( j = 0 ; j <= i ; j++ ) if ( e1[j] != e2[j] ) break;
			if ( j > i ) continue;
			for ( j = 0 ; j <= i ; j++ ) e3[j] = ( e1[j] ? pres->o[j]-e1[j] : 0 );
			for ( ; j < pres->k ; j++ ) e3[j] = 0;

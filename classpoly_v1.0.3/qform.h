...
long qform_order_or_multiple (long *n, long a, long b, long c, long m0, long m1, long L);// given interval [m0,m1] containing order([a,b,c]) compute order or unique-mult of order in interval, return -1 if none, 0 for order, 1 for unique-mult
...
long qform_class_number (long D);       // returns the order of the classgroup using brute force enumeration
long qform_class_exponent (long D);     // returns the exponent of the classgroup
long qform_pgroup_order (long *gens, int n, long p, long q, long L); // given list of  n reduced forms (stored as triples in gens) of p-power order generating subgroup of max order q, computes order
...
// Given e > 0 return the unique multiplie of e in [min,max] if one exists, 0 if none exist, -1 if more than one exist
static inline long unique_multiple (long e, long min, long max)
{
    if ( max-min >= 2*e ) return -1;              // more than 1 multiple in range

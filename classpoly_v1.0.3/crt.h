...
// crt_dir() returns a private per-process scratch directory, created once via
// mkdtemp() under (in order of preference) $CLASSPOLY_TMPDIR, $TMPDIR, or /tmp,
// and removed at exit.  Because every classpoly process gets its own directory,
// the deterministic CRT_*.crt scratch filenames can no longer collide between
// concurrent processes (see crt.c).

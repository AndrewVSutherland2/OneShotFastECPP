...
// Remove the per-process scratch directory (and any files left in it).  In normal
// operation the CRT_*.crt files are deleted as they are consumed, so the directory
// is already empty; this is a backstop for early-exit paths.
static void _crt_dir_cleanup (void)
{
    DIR *d;
    struct dirent *e;
    char buf[2048];

    if ( ! _crt_dir_str[0] ) return;
    if ( (d = opendir (_crt_dir_str)) ) {
        while ( (e = readdir (d)) ) {
            if ( e->d_name[0] == '.' && (! e->d_name[1] || (e->d_name[1] == '.' && ! e->d_name[2])) ) continue;
            snprintf (buf, sizeof(buf), "%s/%s", _crt_dir_str, e->d_name);
            remove (buf);
        }
        closedir (d);
    }
...
// Returns a private per-process scratch directory (created on first use).  The base
// directory is taken from $CLASSPOLY_TMPDIR, then $TMPDIR, then /tmp; a unique
// subdirectory classpoly.XXXXXX is created under it with mkdtemp() so that concurrent
// classpoly processes never share scratch files.  The directory is removed at exit.
char *crt_dir (void)
{
    char *base, tmpl[1024];

    if ( _crt_dir_str[0] ) return _crt_dir_str;
    base = getenv ("CLASSPOLY_TMPDIR");
    if ( ! base || ! base[0] ) base = getenv ("TMPDIR");
    if ( ! base || ! base[0] ) base = "/tmp";
    snprintf (tmpl, sizeof(tmpl), "%s/classpoly.XXXXXX", base);
    if ( ! mkdtemp (tmpl) ) { err_printf ("Unable to create scratch directory under %s (errno %d)\n", base, errno); exit (0); }
    snprintf (_crt_dir_str, sizeof(_crt_dir_str), "%s", tmpl);
    atexit (_crt_dir_cleanup);
    return _crt_dir_str;
}
...
//  for ( i = 0 ; i < ecrt->n ; i++ ) mpz_clear (ecrt->d[i]);  free (ecrt->d);
    free (ecrt->a); free (ecrt->m); free (ecrt->Cdata); free (ecrt->sdata);

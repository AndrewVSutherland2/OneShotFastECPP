...
static inline char *phi_dir (void)
{
	...
	s = getenv("CLASSPOLY_PHI_DIR");							// explicit override of modular polynomial directory
	if ( s && s[0] ) { snprintf (_phi_dir_str, sizeof(_phi_dir_str), "%s", s); return _phi_dir_str; }
	s = getenv("HOME");
